...
class MainProgram
{
     static async Task Main(string[] args)
     {
          ...
          while (true)
          {
               ...
               //Acessando as configurações do .env e configurando ao SMTP
               ...
               //Pegando informalçoes que o usuário inseriu
               string ticker = args[0];
               double venda = double.Parse(args[1]);
               double compra = double.Parse(args[2]);

               //Acessando API e EmaiilSender
               ...
               using (HttpClient client = new HttpClient())
               {
                    ...
                    if (apiResponse?.Results != null && apiResponse.Results.Count > 0)
                    {
                         ...
                         if (historical != null && historical.Count > 0)
                         {
                              ...
                              //Separando dados para montar o gráfico
                              ...
                              if (stock.RegularMarketPrice < venda)
                              {
                                   ...
                                   string parte6 = $"Veja também um gráfico de linha criado para melhor visualização dos preços do {ticker} desse período:\n";
                              ...
                              else if (stock.RegularMarketPrice > compra)
                              {
                                   ...
                                   string parte1 = $"O ativo {ticker} está sendo cotado a {stock.RegularMarketPrice}. Recomendação: VENDA.\n\n";
                                   string parte2 = $"Caso queira ana lisar de maneira mais profunda observe os dados do {ticker} nos últimos 7 dias, 30 dias e 3 meses: \n";
                                   string parte3 = $"07 dias    => Valor mínimo: {min7Days} ; Valor máximo: {max7Days} ; Média do período: {avg7Days}\n";
                                   string parte4 = $"30 dias    => Valor mínimo: {min30Days} ; Valor máximo: {max30Days} ; Média do período: {avg30Days}\n";
                                   string parte5 = $"03 meses => Valor mínimo: {min} ; Valor máximo: {max} ; Média do período: {avg}\n\n";
                                   string parte6 = $"Veja também um gráfico de linha criado para melhor visualização dos preços do {ticker} nesse período:";
                                   string body = $"{parte1} {parte2} {parte3} {parte4} {parte5} {parte6}";
                                   emailSender.SendEmail(from, to, subject, body, anexos);
                              }
                              else
